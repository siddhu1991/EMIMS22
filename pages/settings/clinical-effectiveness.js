...
// Table Columns
import columns from '../../table-columns/clinical-effectiveness';
// Utils
import { fDate } from '../../utils/formatTime';
// Database
import { supabase } from '../../utils/supabaseClient';
// Models
import { addCE, updateCE } from '../../models/clinical-effectiveness';
// ----------------------------------------------------------------------
...
ClinicalEffectiveness.getLayout = function getLayout(page) {
  return <Layout variant="main">{page}</Layout>;
};
export default function ClinicalEffectiveness({ rows }) {
  ...
  const handleRowEditCommit = React.useCallback(
    async (id) => {
      ...
      if (oldRow.isNew) {
        const inserted = await addCE(newRow);
        ...
        if (inserted.error) {
          ...
          enqueueSnackbar('Clinical Effectiveness Inserted Successfully!', { variant: 'success' });
        }
      } else {
        const updated = await updateCE(id, newRow, fDate(Date.now()));
        if (updated.error) {
          apiRef.current.updateRows([oldRow]);
          enqueueSnackbar('Something went wrong, Please try again!', { variant: 'error' });
        } else {
          apiRef.current.updateRows([{ id, updated_at: fDate(updated.data[0].updated_at) }]);
          enqueueSnackbar('Clinical Effectiveness updated Successfully!', { variant: 'success' });
        }
      }
    },
    [apiRef]
  ...
  return (
    <Page title="Clinical Effectiveness">
      <Container maxWidth="false">
        <PageHeading heading="Clinical Effectiveness" />
        <Card>
          <DataGridWithEdit
            rows={rows}
...
export async function getServerSideProps() {
  let query = supabase.from('ems_clinical_effectiveness').select('*');
