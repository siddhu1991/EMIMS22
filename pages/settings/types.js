...
// Table Columns
import columns from '../../table-columns/types';
// Utils
import { fDate } from '../../utils/formatTime';
// Database
import { supabase } from '../../utils/supabaseClient';
// Models
import { addType, updateType } from '../../models/types';
// ----------------------------------------------------------------------
...
Types.getLayout = function getLayout(page) {
  return <Layout variant="main">{page}</Layout>;
};
export default function Types({ rows }) {
  ...
  function EditToolbar(props) {
    ...
    const handleClick = () => {
      ...
      apiRef.current.updateRows([
        { id, isNew: true, name:'', alert: false, review: false, updated_at: Date.now(), created_at: Date.now() },
      ]);
  ...
  const handleRowEditCommit = React.useCallback(
    async (id) => {
      ...
      if (oldRow.isNew) {
        const inserted = await addType(newRow);
          ...
          apiRef.current.updateRows([
            {
              ...
              alert: newRow.alert.value,
              review: newRow.review.value,
              updated_at: fDate(inserted.data[0].updated_at),
              created_at: fDate(inserted.data[0].created_at),
            },
          ]);
          enqueueSnackbar('Incident Type Inserted Successfully!', { variant: 'success' });
        }
      } else {
        const updated = await updateType(id, newRow, fDate(Date.now()));
        if (updated.error) {
          apiRef.current.updateRows([oldRow]);
          enqueueSnackbar('Something went wrong, Please try again!', { variant: 'error' });
        } else {
          apiRef.current.updateRows([{ id, updated_at: fDate(updated.data[0].updated_at) }]);
          enqueueSnackbar('Incident Type updated Successfully!', { variant: 'success' });
        }
      }
    },
    [apiRef]
  ...
  return (
    <Page title="Incident Types">
      <Container maxWidth="false">
        <PageHeading heading="Incident Types" />
        <Card>
          <DataGridWithEdit
            rows={rows}
...
export async function getServerSideProps() {
  let query = supabase.from('ems_incident_types').select('*');
