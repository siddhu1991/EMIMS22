...
// Table Columns
import columns from '../../table-columns/severity';
// Utils
import { fDate } from '../../utils/formatTime';
// Database
import { supabase } from '../../utils/supabaseClient';
// Models
import { addSeverity, updateSeverity } from '../../models/severity';
// ----------------------------------------------------------------------
...
Severity.getLayout = function getLayout(page) {
  return <Layout variant="main">{page}</Layout>;
};
export default function Severity({ rows }) {
  ...
  function EditToolbar(props) {
    ...
    const handleClick = () => {
      ...
      apiRef.current.updateRows([{ id, isNew: true, name: '', alert: false, review: false, updated_at: Date.now(), created_at: Date.now() }]);
  ...
  const handleRowEditCommit = React.useCallback(
    async (id) => {
      ...
      if (oldRow.isNew) {
        const inserted = await addSeverity(newRow);
        ...
        if (inserted.error) {
          ...
          enqueueSnackbar('Severity Inserted Successfully!', { variant: 'success' });
        }
      } else {
        const updated = await updateSeverity(id, newRow, fDate(Date.now()));
        if (updated.error) {
          apiRef.current.updateRows([oldRow]);
          enqueueSnackbar('Something went wrong, Please try again!', { variant: 'error' });
        } else {
          apiRef.current.updateRows([{ id, updated_at: fDate(updated.data[0].updated_at) }]);
          enqueueSnackbar('Severity updated Successfully!', { variant: 'success' });
        }
      }
    },
    [apiRef]
...
export async function getServerSideProps() {
  let query = supabase.from('ems_severity').select('*');
